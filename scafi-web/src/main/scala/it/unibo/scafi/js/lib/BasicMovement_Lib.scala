  ...
  trait Movement2D extends ProgramMovementImplicits {
    ...
    def explore(zone : Zone, trajectoryTime : Int, reachGoalRange : Double = 0) : Velocity = {
      ...
      val (_, _, velocity) = rep((randomCoordZone(zone), trajectoryTime, Velocity.Zero)){
        ...
        case (goal, decay, v) if (goal.distance(currentPosition()) < reachGoalRange)=> (goal, 0, goToPoint(goal.x, goal.y))
    ...
    private def randomCoordZone(zone : Zone) : Point2D = zone match {
      case CircularZone((cx, cy), radius) =>  Point2D(cx + radius * positiveNegativeRandom(), cy + radius * positiveNegativeRandom())
      case RectangularZone((rx, ry), w, h) => Point2D(rx + (w / 2) * positiveNegativeRandom(), ry + (h / 2) * positiveNegativeRandom())
    }
