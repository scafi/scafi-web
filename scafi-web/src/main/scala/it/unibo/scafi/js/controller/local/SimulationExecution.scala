...
/**
  ...
  * a "Daemon" (i.e. the simulation go on by itself in background)
  */
...
object SimulationExecution {
  ...
  /**
    ...
    */
  case class TickBased(batchSize: Int = 1,
                       protected val exec: (Int) => Future[Unit])
    ...
    /**
      ...
      */
    def toDaemon(delta: Int = 0, batchSize: Int = this.batchSize): Daemon =
      Daemon(batchSize, delta, exec)

    /**
      * alter the batch size of the current execution
      * @return the new instance of execution
      */
    def withBatchSize(batchSize: Int): TickBased =
      this.copy(batchSize = batchSize)
  }
  ...
  /**
    ...
    */
  case class Daemon(batchSize: Int = 1,
                    delta: Int = 0,
                    protected val exec: (Int) => Future[Unit])
      extends SimulationExecution {
