...
package object utils {
  ...
  implicit class NullableOps[A](value : Nullable[A]) {
    ...
    /**
      ...
      * @param ifEmpty the function called if nullable is null
      * @param f the function called if nullable is some
      * @tparam B the output type
      * @return the result of ifEmpty if nullable is null f otherwise
      */
    @inline def fold[B](ifEmpty: => B)(f: A => B): B = if (isDefined) f(forceGet) else ifEmpty

    /**
      * map operation like Option.map[B]
      * @param f the mapping function to act only if nullable is defined
      * @tparam B the result type of mapping operation
      * @return null is isn't defined f(value) otherwise
      */
    @inline def map[B](f: A => B): B | Null = fold[B | Null](null)(f(_))

    /**
      * get or else like Option.getOrElse[B].
      * @param ifEmpty value in case of the Nullable is null.
      * @tparam B the result type of get or else.
      * @return ifEmpty if the nullable isn't defined, value otherwise
      */
    ...
    /**
      * like Option.foreach
      * @param f the function called if the nullable is defined
      */
