...
object Execution {
  ...
  /**
    ...
    * @param delta the period in which the handler will be executed
    * @param fun the handler to execute at certain period
    */
  def schedule(delta : Int)(fun : => Unit) : JavascriptHandler = JavascriptIntervalHandler(js.timers.setInterval(delta){fun})
