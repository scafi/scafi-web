...
describe("SvgSimulationRenderer", () => {
  ...
  it("can be instantiated and mounts clean container markup", () => {
    const renderer = new SvgSimulationRenderer();
    renderer.mount(parent);

    const container = parent.querySelector<HTMLElement>(".graph-workspace-container");
  ...
  it("renders nodes and edges in the SVG graph viewport when loaded", () => {
    ...
    expect(edges[0].getAttribute("data-edge-key")).toBe("node-1->node-2");
