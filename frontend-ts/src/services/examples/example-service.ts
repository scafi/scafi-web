...
function resolveExampleRenderer(groupName: string, example: ExampleDefinition, sensors: string[]): string {
  ...
  switch (example.name) {
    ...
    case "Custom temperature renderer":
      return [
        "/**",
        " * ==========================================",
        " * CUSTOM TEMPERATURE MONITORING RENDERER",
        " * ==========================================",
        " * ",
        " * This custom renderer is defined dynamically on-the-fly inside the script.",
        " * It demonstrates how to retrieve local node sensor values and draw completely",
        " * custom node fills, rings, and text overlays without being restricted",
        " * to default plugins.",
        " */",
        "",
        "return {",
        ...
        "  fontSize: 12,",
        "  showId: false,",
        "  rendererType: \"lightweight\", // supports both standard (SVG) and lightweight (Canvas)",
        "  ",
        "  renderNode(context) {",
        "    // Retrieve local temperature sensor value (or fall back to average default)",
        "    const temp = context.node.labels.temperature ?? 24.5;",
        "    ",
        "    // Color gradient mapping: blue (cold, 10°C) to red (hot, 40°C)",
        "    const minTemp = 10.0;",
        "    const maxTemp = 40.0;",
        "    const ratio = Math.max(0, Math.min(1, (temp - minTemp) / (maxTemp - minTemp)));",
        "    const hue = (1.0 - ratio) * 240.0; // HSL Hue: 240 is blue, 0 is red",
        "    const color = `hsl(${Math.round(hue)} 85% 55%)`;",
        "    ",
        "    // Dynamically scale node size depending on temperature",
        "    const dynamicNodeSize = context.defaults.nodeSize + (ratio * 6.5);",
        "    ",
        "    return {",
        "      fill: color,",
        "      nodeSize: dynamicNodeSize,",
        "      overlays: [",
        "        // 1. Draw a customized ambient heat halo/ring around the node",
        "        {",
        "          kind: \"ring\",",
        "          radius: dynamicNodeSize + 5.5,",
        "          stroke: color,",
        "          strokeWidth: 1.5,",
        "          fill: color,",
        "          fillOpacity: 0.08,",
        "        },",
        "        // 2. Render the precise temperature value on top of the node",
        "        {",
        "          kind: \"text\",",
        "          text: `${temp.toFixed(1)}°C`,",
        "          y: -(dynamicNodeSize + 11.0),",
        "          fill: \"#ffffff\",",
        "          fontSize: 10.5,",
        "          anchor: \"middle\",",
        "        }",
