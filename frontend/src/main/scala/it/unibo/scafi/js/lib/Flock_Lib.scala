...
/** Flocking behaviour from Craig Reynolds (https://www.red3d.com/cwr/index.html) implementation taken from
  * https://gamedevelopment.tutsplus.com/tutorials/3-simple-rules-of-flocking-behaviors-alignment-cohesion-and-separation--gamedev-3444
  * other link : https://www.red3d.com/cwr/boids/ This library describes flocking like behaviour in the Aggregate
  * Computing context.
  */
trait Flock_Lib {
  ...
  trait FlockLib extends ProgramMovementImplicits {
    ...
    /** This class summarize the parameters to describe a Flocking like behaviour.
      * @param flockingField
      *   the boolean field in which the flock behaviour should happen
      * @param attractionForce
      *   the weight of the attraction vector, default 1.0
      * @param alignmentForce
      *   the weight of the alignment vector, default 1.0
      * @param repulsionForce
      *   the weight of the repulsion vector, default 1.0
      * @param separationDistance
      *   the threshold used by repulsion force evaluation to consider two nodes as neighbours
      * @param otherVelocityEvaluation
      *   a sequence of other velocities evaluation that should influence boids (e.g. a goal position, the wind,...)
      */
    case class FlockBehaviour(
        flockingField: Boolean = true,
      ...
      /** Using aggregate computing syntax, it returns a velocity field that simulate a flock behaviour according to the
        ...
        * @return
        *   the velocity field (as unary vector) updating through time.
        */
      def run(): Velocity = {
        rep[Velocity](Velocity.Zero) { // it is a differential equation, I need to maintain the current "velocity" field.
          v =>
            val neighbourhoodField =
              getValuesFromActiveNode(flockingField)(currentPosition()) // get a field of neighbourhood position
            val mainVector = List(
    ...
    /** It has the opposite behaviour of flock. This is observed in nature when the swarm escape from an enemy (e.g.
      * predator)
      */
    def antiflock(): Velocity = FlockBehaviour(attractionForce = -1, alignmentForce = -1, repulsionForce = -1).run()
    /** an utility method used to move a swarm of element with some velocity, maintaining a separation distance between
      * elements. It is similar to use FlockBehaviour(attractionForce = 0.0, alignmentForce = 0.0, separationDistance,
      * otherVelocityEvaluation(velocity)). However, in this case, the velocity is weighted according to the number of
      * neighbours. In this way, the separation behaviour is always preferred.
      * @param selector
      *   the boolean field in which the separation behaviour should happen
      * @param velocity
      *   the desired velocity field
      * @param separationDistance
      *   the target distance between each node.
      * @return
      *   a unit vector based on velocity evalutaiton and separation distance.
      */
    ...
    def withSeparation(velocity: Velocity)(separationDistance: Double): Velocity =
      withSeparation(selector = true)(velocity)(separationDistance)

    /* steer towards the average heading of local flockmates */
    private[Flock_Lib] def alignment(flockingSensor: => Boolean, velocity: Velocity, neighbourCount: Int): Velocity = {
      val averageVelocity: Velocity =
        getValuesFromActiveNode(flockingSensor)(velocity).fold(Velocity.Zero)(_ + _) / neighbourCount
  ...
  trait AdvancedFlock {
    self: FlockLib
      with FlockLib.Dependencies
      with Movement2D => // a way to define component dependencies, using a companion object with a type alias.
    ...
    implicit class RichFlock(flock: FlockBehaviour) {
      ...
      def addBehaviour(v: => Velocity): FlockBehaviour =
        flock.copy(otherVelocityEvaluation = (() => v) :: flock.otherVelocityEvaluation)
      /** It simulates a constant force (a sort of wind) that guide the swarm to a direction
        * @param v
        *   the wind direction (express as velocity)
        * @return
        *   the behaviour with the wind influence
        */
      ...
      /** It guide the swarm to a specific position (a global goal)
        * @param goal
        *   the target position where the swarm want to move on
        * @param importance
        *   how much the goal influences the swarm? (default 1.0)
        * @return
        *   the behaviour with the goal in mind
        */
      def withGoal(goal: (Double, Double), importance: Double = 1.0): FlockBehaviour = flock.addBehaviour {
        goToPoint(goal._1, goal._2) * importance
      }
      /** It guide the swarm to avoid a specific part of the field (delimited with obstacleSensor selector)
        * @param obstacleSensor
        *   the field that the swarm tends to avoid
        * @param force
        *   how much the obstacle avoidance influences the swarm? (default 1.0)
        * @return
        *   the behaviour with the obstacle avoidance influence
        */
