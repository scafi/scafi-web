...
object ScastieClient {
  ...
  private def scala2Target = s"""{"Scala2":{"scalaVersion":"$scalaV"}}"""
  private def jsTarget = s"""{"Js":{"scalaVersion":"$scalaV","scalaJsVersion":"$scalaJsV"}}"""

  private def dep(groupId: String, artifact: String): String =
    s"""{"groupId":"$groupId","artifact":"$artifact","target":$scala2Target,"version":"$scafiVersion","isAutoResolve":true}"""
  ...
  def compile(code: String, mode: Mode): Future[String] = {
    ...
    val wrappedCode = wrapCode(core)
    val payload = buildPayload(wrappedCode)

    for {
      snippetId <- postRun(payload)
      jsContent <- waitForJs(snippetId)
    } yield jsContent // No injector call needed; standalone module self-initializes
  }

  private def buildPayload(code: String): String = {
    val escaped = JSON.stringify(code)
    val libs = Seq(
      dep("it.unibo.scafi", "scafi-core"),
      dep("it.unibo.scafi", "scafi-commons"),
      dep("it.unibo.scafi", "scafi-simulator")
    ).mkString("[", ",", "]")
    s"""{"SbtInputs":{"isWorksheetMode":false,"code":$escaped,"target":$jsTarget,"libraries":$libs,"librariesFromList":[],"sbtConfigExtra":"","sbtPluginsConfigExtra":"","isShowingInUserProfile":false}}"""
  }
  ...
  private def wrapCode(core: String): String =
